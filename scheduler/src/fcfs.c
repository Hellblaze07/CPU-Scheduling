    ...
    while (completed < num_processes) {
        ...
        if (idx != running_idx) {
            if (running_idx != -1 && processes[running_idx].remaining_time > 0) {
                record_event(&result.trace, current_time, EVENT_PREEMPTION, processes[running_idx].pid, 0, 0, 0, 0, "Preempted"); // Should not happen in FCFS
            }
            if (idx != -1) {
                context_switches++;
                record_event(&result.trace, current_time, EVENT_DISPATCH, processes[idx].pid, 0, 0, 0, 0, "FCFS Dispatch");
