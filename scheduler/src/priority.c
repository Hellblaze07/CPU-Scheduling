...
struct Result* priority_scheduler(struct Process processes[], int num_processes) {
    ...
    while (completed < num_processes) {
        ...
        for (int i = 0; i < num_processes; i++) {
            if (processes[i].arrival_time <= current_time && processes[i].remaining_time > 0 && processes[i].priority < highest) {
        ...
        if (idx != running_idx) {
            ...
            if (idx != -1) {
                context_switches++;
                char reason[50];
                sprintf(reason, "Priority %d", processes[idx].priority);
                record_event(&result.trace, current_time, EVENT_DISPATCH, processes[idx].pid, 0, 0, 0, 0, reason);
